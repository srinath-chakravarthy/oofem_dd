...
namespace oofem {
REGISTER_Material(ExpCZMaterial);
...
*/
    ...
    //std::cout << "AA: " << answer.at(1) << " " << answer.at(2) << " " << answer.at(3) << "\n";
...
ExpCZMaterial :: give3dStiffnessMatrix_Eng(FloatMatrix &answer, MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep)
{
    ...
    if ( rMode == TangentStiffness ) {
        ...
        answer.at(1, 3) = 
        answer.at(3, 1) = (-xin + (1. - q) / (r - 1.) * (r - xin)) *
                          (exp(-xin) * exp(-xit * xit)) *
                          (2. * GIc / (gn0 * gn0)) *
                          xit;
        answer.at(1, 1) = (q + xin * (r - q) / (r - 1.)) *
                          (exp(-xin) * exp(-xit * xit)) *
                          (2. * GIc / (gs0 * gs0)) *
                          (1. - 2. * (xit * xit));
    }  else {
